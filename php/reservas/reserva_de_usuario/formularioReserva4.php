...
if (insertarReserva($rela_horario, $fecha, $rela_zona, $rela_persona, $monto_pagado, $monto_total)) {
	$rela_sucursal = $conexion->query("SELECT id_sucursal FROM sucursal JOIN zona ON rela_sucursal = id_sucursal WHERE id_zona = $rela_zona");
	$idSucursal = $rela_sucursal->fetch_assoc();
	$rela_sucursal = $idSucursal['id_sucursal'];
	$queryNotificacion = "INSERT INTO notificacion (mensaje, estado, rela_sucursal)
							VALUES ('persona $rela_persona ah reservado','no leido',$rela_sucursal)";
