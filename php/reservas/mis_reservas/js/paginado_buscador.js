$(document).ready(function() {
    ...
    function cargarTabla(id_persona,filtro = '', pagina = 1) {
        $.ajax({
            url: 'ajax/obtener_reservas.php',
            type: 'GET',
            data: { filtro: filtro, pagina: pagina , id_persona: id_persona},
            dataType: 'json',
    ...
    cargarTabla(id_persona);

    // Evento de búsqueda
    $('#buscador').on('keyup', function() {
        var filtro = $(this).val();
        cargarTabla(id_persona ,filtro); //llamar a la funcion con el termino de busqueda
    });
    ...
    $(document).on('click', '.pagina-boton', function() {
        ...
        cargarTabla(id_persona,filtro, page);
    });
